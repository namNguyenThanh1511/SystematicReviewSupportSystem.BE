...
namespace SRSS.IAM.Repositories.PaperCitationRepo
{
    public interface IPaperCitationRepository : IGenericRepository<PaperCitation, Guid, AppDbContext>
    {
        ...
        Task<Dictionary<Guid, int>> CountByTargetsAsync(IEnumerable<Guid> targetPaperIds, CancellationToken cancellationToken = default);
        Task<Dictionary<Guid, int>> CountBySourcesAsync(IEnumerable<Guid> sourcePaperIds, CancellationToken cancellationToken = default);
